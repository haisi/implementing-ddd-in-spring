...
class TestTests {
    ...
    @ArchTest
    private final ArchRule aggregatesHaveTestFixtures = ArchRuleDefinition
            .classes().that()
    ...
    private static class RepositoryHasInMemoryImplementation extends ArchCondition<JavaClass> {
        ...
        public void check(JavaClass repository, ConditionEvents events) {
            ...
            String inMemoryRepoClassName = repository.getPackageName() + ".InMemory" + repository.getSimpleName();
            try {
                Class<?> inMemoryRepoClass = Class.forName(inMemoryRepoClassName);
                if (classDoesNotImplementItem(inMemoryRepoClass, repository)) {
                    events.add(SimpleConditionEvent.violated(repository,
                            String.format("'%s' does not implement its repository interface '%s'", inMemoryRepoClassName, repository.getName())));
                } else if (classDoesNotImplementInterface(inMemoryRepoClass, DeepCopyAggregateForInMemoryRepository.class)) {
                    events.add(SimpleConditionEvent.violated(repository,
                            String.format("'%s' does not implement interface '%s'", inMemoryRepoClassName, DeepCopyAggregateForInMemoryRepository.class.getSimpleName())));
                }
            } catch (ClassNotFoundException e) {
                events.add(SimpleConditionEvent.violated(repository,
                        String.format("Could not find in-memory implementation '%s' for repository '%s'", inMemoryRepoClassName, repository.getName())));
            }
        }
    }

    private static boolean classDoesNotImplementInterface(Class<?> implementation, Class<?> interfazeToImpl) {
    ...
    static final Predicate<JavaClass> TEST_LOCATION = MAVEN_TEST_PATTERN.or(GRADLE_TEST_PATTERN).or(INTELLIJ_TEST_PATTERN);
